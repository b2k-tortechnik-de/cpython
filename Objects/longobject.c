...
_PyLong_New(Py_ssize_t size)
{
    ...
    if (size <= 1) {
        ...
        result = (PyLongObject *)_PyFreeList_Alloc(&interp->small_int_freelist);
#else
    if (size == 0) {
...
#if WITH_FREELISTS
    PyInterpreterState *interp = _PyInterpreterState_GET();
    PyLongObject *v = (PyLongObject *)_PyFreeList_Alloc(&interp->small_int_freelist);
#else
...
int_dealloc(PyLongObject *op)
{
    ...
    if (PyLong_CheckExact(op) && IS_MEDIUM_VALUE(op)) {
        PyInterpreterState *interp = _PyInterpreterState_GET();
        _PyFreeList_Free(&interp->small_int_freelist, op);
    }
    else
#endif
    {
